...
int32_t verifyResponsePacket(kdbgerCommPkt_t* pKdbgerCommPkt,
  ...
  switch (pKdbgerCommPkt->kdbgerCommHdr.opCode) {
    ...
    case KDBGER_RSP_MEM_READ:
      if (op != KDBGER_REQ_MEM_READ) return 1;
      break;

    case KDBGER_RSP_MEM_WRITE:
      if (op != KDBGER_REQ_MEM_WRITE) return 1;
      break;

    case KDBGER_RSP_IO_READ:
      if (op != KDBGER_REQ_IO_READ) return 1;
      break;

    case KDBGER_RSP_IO_WRITE:
      if (op != KDBGER_REQ_IO_WRITE) return 1;
      break;

    case KDBGER_RSP_PCI_READ:
      if (op != KDBGER_REQ_PCI_READ) return 1;
      break;

    case KDBGER_RSP_PCI_WRITE:
      if (op != KDBGER_REQ_PCI_WRITE) return 1;
      break;

    case KDBGER_RSP_IDE_READ:
      if (op != KDBGER_REQ_IDE_READ) return 1;
      break;

    case KDBGER_RSP_IDE_WRITE:
      if (op != KDBGER_REQ_IDE_WRITE) return 1;
      break;

    case KDBGER_RSP_CMOS_READ:
      if (op != KDBGER_REQ_CMOS_READ) return 1;
      break;

    case KDBGER_RSP_CMOS_WRITE:
      if (op != KDBGER_REQ_CMOS_WRITE) return 1;
      break;

    case KDBGER_RSP_PCI_LIST:
      if (op != KDBGER_REQ_PCI_LIST) return 1;
      break;

    case KDBGER_RSP_E810_LIST:
      if (op != KDBGER_REQ_E810_LIST) return 1;
      break;
...
int32_t executeFunction(int32_t fd, kdbgerOpCode_t op, uint64_t addr,
  ...
  switch (op) {
    ...
    case KDBGER_REQ_IO_WRITE:
      ...
      pKdbgerCommPkt->kdbgerReqIoWritePkt.address =
          (uint16_t)(addr & 0xFFFFULL);
    ...
    case KDBGER_REQ_PCI_READ:
      ...
      pKdbgerCommPkt->kdbgerReqPciReadPkt.address =
    ...
    case KDBGER_REQ_PCI_LIST:
      ...
      pKdbgerCommPkt->kdbgerCommHdr.pktLen = sizeof(kdbgerReqPciWritePkt_t);
      break;

    case KDBGER_REQ_E810_LIST:

      pKdbgerCommPkt->kdbgerCommHdr.pktLen = sizeof(kdbgerReqE820ListPkt_t);
      break;
