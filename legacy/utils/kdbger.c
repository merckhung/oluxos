...
void updateStatusTimer(kdbgerUiProperty_t* pKdbgerUiProperty) {
  ...
  if (pKdbgerUiProperty->kdbgerBasePanel.lastSecond < thisSecond) {
    ...
    printWindowAt(pKdbgerUiProperty->kdbgerBasePanel, time, KDBGER_STRING_NLINE,
                  KDBGER_MAX_TIMESTR, KDBGER_MAX_LINE,
                  KDBGER_MAX_COLUMN - KDBGER_MAX_TIMESTR, BLACK_WHITE,
                  "%2.2d:%2.2d:%2.2d", nowtime->tm_hour, nowtime->tm_min,
                  nowtime->tm_sec);
  ...
  if (pKdbgerUiProperty->kdbgerBasePanel.statusStr &&
    ...
    if ((len - pKdbgerUiProperty->kdbgerBasePanel.strIdx) >
        (KDBGER_MAX_COLUMN - KDBGER_MAX_TIMESTR))
      len = KDBGER_MAX_COLUMN - KDBGER_MAX_TIMESTR;

    snprintf(stsBuf, len, "%s",
             (pKdbgerUiProperty->kdbgerBasePanel.statusStr +
              pKdbgerUiProperty->kdbgerBasePanel.strIdx));

    // Update status bar
    printWindow(pKdbgerUiProperty->kdbgerBasePanel, status, KDBGER_STRING_NLINE,
                KDBGER_MAX_COLUMN - KDBGER_MAX_TIMESTR, KDBGER_MAX_LINE,
                KDBGER_MIN_COLUMN, BLACK_WHITE, "%s", stsBuf);

    pKdbgerUiProperty->kdbgerBasePanel.strIdx++;
    update = 1;
  }

  if (update) pKdbgerUiProperty->kdbgerBasePanel.lastSecond = thisSecond;
}

void printBasePlane(kdbgerUiProperty_t* pKdbgerUiProperty) {
  // Background Color
  printWindow(pKdbgerUiProperty->kdbgerBasePanel, background, KDBGER_MAX_LINE,
              KDBGER_MAX_COLUMN, KDBGER_MIN_LINE, KDBGER_MIN_COLUMN, WHITE_BLUE,
              "");

  // Logo
  printWindow(pKdbgerUiProperty->kdbgerBasePanel, logo, KDBGER_STRING_NLINE,
              KDBGER_MAX_COLUMN, KDBGER_MIN_LINE, KDBGER_MIN_COLUMN, WHITE_RED,
              KDBGER_PROGRAM_FNAME);

  // Copyright
  printWindow(pKdbgerUiProperty->kdbgerBasePanel, copyright,
              KDBGER_STRING_NLINE, strlen(KDBGER_AUTHER_NAME), KDBGER_MIN_LINE,
              KDBGER_MAX_COLUMN - strlen(KDBGER_AUTHER_NAME), WHITE_RED,
              KDBGER_AUTHER_NAME);
...
";
  ...
  if (pKdbgerUiProperty->kdbgerBasePanel.toggleHelp) {
    printWindowMove(pKdbgerUiProperty->kdbgerBasePanel, help,
                    (KDBGER_MAX_LINE - 4), (KDBGER_MAX_COLUMN - 4),
                    KDBGER_HELP_X_POS, KDBGER_HELP_Y_POS, BLACK_WHITE, "%s",
                    helpTxt);
  } else
...
int32_t main(int32_t argc, int8_t** argv) {
  ...
  kdbgerUiProperty.kdbgerHwFunc = kdbgerUiProperty.kdbgerPreviousHwFunc =
      KHF_INIT;
  ...
  for (;;) {
    ...
    switch (kdbgerUiProperty.inputBuf) {
      ...
      // Help
      case KBPRS_F1:
        kdbgerUiProperty.kdbgerBasePanel.toggleHelp =
            !kdbgerUiProperty.kdbgerBasePanel.toggleHelp;
        break;
  ...
  DirectTransition:
    ...
    switch (kdbgerUiProperty.kdbgerHwFunc) {
      ...
      case KHF_IO:
        handleKeyPressForDumpPanel(&kdbgerUiProperty);
        if (!readIo(&kdbgerUiProperty)) printDumpUpdatePanel(&kdbgerUiProperty);
        break;
