...
int thread_ipc_recv(u32 src, void *buf, u32 size) {
    ...
    if (src != ANY_THREAD) {
        if (src == current_thread->tid || src >= MAX_THREADS || threads[src].state == THREAD_STATE_FREE) {
            IntEnable();
            return -1;
        }
    }
    ...
    if (src == ANY_THREAD) {
        ...
        if (t->state == THREAD_STATE_BLOCKED && t->ipc_partner == current_thread->tid) {
            sender_thread = t;
        }
    }
