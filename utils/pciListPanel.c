...
void printPciListBasePanel(kdbgerUiProperty_t* pKdbgerUiProperty) {
  // Title
  printWindowAt(pKdbgerUiProperty->kdbgerPciListPanel, title,
                KDBGER_PCIL_TITLE_LINE, KDBGER_PCIL_TITLE_COLUMN,
                KDBGER_PCIL_TITLE_X_POS, KDBGER_PCIL_TITLE_Y_POS, YELLOW_BLACK,
                KDBGER_PCIL_TITLE);
}
...
void printPciListUpdatePanel(kdbgerUiProperty_t* pKdbgerUiProperty) {
  ...
  for (i = start; i < end; i++) {
    ...
    p += snprintf(p, (KDBGER_BUF_SIZE - (p - buf)), KDBGER_PCIL_LINE_FMT,
    ...
    if (i == (pKdbgerUiProperty->kdbgerPciListPanel.hlIndex +
              pKdbgerUiProperty->kdbgerPciListPanel.pageOffset))
      snprintf(hlbuf, KDBGER_PCIL_CON_COLUMN + 1, KDBGER_PCIL_LINE_FMT,
               (pKdbgerUiProperty->pKdbgerPciIds + i)->venTxt,
               (pKdbgerUiProperty->pKdbgerPciIds + i)->devTxt,
               (pKdbgerUiProperty->pKdbgerPciDev + i)->vendorId,
               (pKdbgerUiProperty->pKdbgerPciDev + i)->deviceId,
  ...
  // Update the screen
  printWindowAt(pKdbgerUiProperty->kdbgerPciListPanel, content,
                KDBGER_PCIL_CON_LINE, KDBGER_PCIL_CON_COLUMN,
                KDBGER_PCIL_CON_X_POS, KDBGER_PCIL_CON_Y_POS, WHITE_BLUE, "%s",
                buf);

  // Highlight
  printWindowMove(
      pKdbgerUiProperty->kdbgerPciListPanel, highlight, KDBGER_STRING_NLINE,
      KDBGER_PCIL_CON_COLUMN,
      KDBGER_PCIL_CON_X_POS + pKdbgerUiProperty->kdbgerPciListPanel.hlIndex,
      KDBGER_PCIL_CON_Y_POS, BLACK_CYAN, "%s", hlbuf);
}
...
s32 handleKeyPressForPciListPanel(kdbgerUiProperty_t* pKdbgerUiProperty) {
  switch (pKdbgerUiProperty->inputBuf) {
    case KBPRS_UP:
      ...
      else if (pKdbgerUiProperty->kdbgerPciListPanel.pageOffset)
        pKdbgerUiProperty->kdbgerPciListPanel.pageOffset--;
      break;
    ...
    case KBPRS_DOWN:
      ...
      if (pKdbgerUiProperty->kdbgerPciListPanel.hlIndex <
        ...
        pKdbgerUiProperty->kdbgerPciListPanel.hlIndex++;
                ...
                KDBGER_REC_PER_PAGE) < pKdbgerUiProperty->numOfPciDevice)
        pKdbgerUiProperty->kdbgerPciListPanel.pageOffset++;
      break;

    case KBPRS_PGUP:

      if (pKdbgerUiProperty->numOfPciDevice < KDBGER_REC_PER_PAGE) break;
