...
static void Usage(void) {
  fprintf(stderr,
          "\nCopyright (c) 2008 Olux Organization All rights reserved.\n\n");
  fprintf(stderr,
          "OluxOS Kernel Image Installer, Version " OLUX_VERSION "\n\n");

  fprintf(stderr, "Authors: Merck Hung <merck@olux.org>\n\n");

  fprintf(stderr,
          "Usage: instkrn -k <KERNEL_NAME> -d <HDD_NAME> ( [-a <ADDR> ] | "
          "-[hvr] )\n\n");
  ...
  fprintf(stderr,
          "\t-a\tSpecify a address for original bootloader, ex: 0x7A00\n");
  fprintf(
      stderr,
...
int32_t main(int32_t argc, int8_t** argv) {
  ...
  if (OLUX_SZ_SECTOR != rwbytes) {
    fprintf(stderr,
            "Error occurred while reading original bootloader (%d != %d)\n",
